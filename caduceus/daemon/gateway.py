    ...
    def _serve(self, control_app, aigateway_app, services) -> None:  # pragma: no cover
        ...
        async def _run():
            ...
            try:
                ...
                # Graceful shutdown is container-safe (BR-O2): stop supervision and close
                # pooled HTTP transports only — NEVER `docker stop`/`rm`, so agent
                # containers keep running and reconnect on the next start.
                await services.supervisor.stop()
                await services.agent_service.await_jobs(timeout=5.0)  # let in-flight creates settle
                await services.chat_service.close_all()  # close pooled HTTP transports
