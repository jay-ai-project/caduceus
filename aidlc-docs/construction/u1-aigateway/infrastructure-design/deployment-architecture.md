...
flowchart LR
    subgraph HOST["host (WSL2)"]
        ...
        LS["llama-swap<br/>127.0.0.1:9292"]
        CLI["caduceus CLI"]
    end
    ...
    AIGW -->|"http://localhost:9292/v1"| LS
...
| Failure | Behavior |
|---|---|
| Upstream (llama-swap) down | `/v1/*` → 502; `/v1/models` → minimal `[default]`; daemon stays up |
...
- Integration: bring up daemon, hit AI-Gateway from a real sandbox, assert streamed completion via llama-swap.
