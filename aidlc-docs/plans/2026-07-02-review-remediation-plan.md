...
## Spike Notes (Phase 3a, 2026-07-02 — hermes 0.17.0 소스 `~/.hermes/hermes-agent` 확인)
- **config.yaml 스키마** (`cli-config.yaml.example` + 소스): top-level 키 = model/terminal/browser/
  tool_loop_guardrails/compression/prompt_caching/memory/skills/agent/platform_toolsets/… .
  **`soul:` 키는 config에 없음** — soul은 `HERMES_HOME/SOUL.md`(컨테이너 `/opt/data/SOUL.md`)
  파일이며, prompt-build 시마다 읽힘(prompt_builder.py:1797) → **변경 즉시 다음 턴 반영, 재시작 불필요**.
- **skills** = `HERMES_HOME/skills/` 하위 디렉터리들. prompt-build마다 mtime-manifest로 재스캔
  (prompt_builder.py:1276) → 파일 변경 즉시 반영, 재시작 불필요. **단, 스킬 "추가"는 콘텐츠
  (SKILL.md)가 필요해 이름만으로는 생성 불가 → `--add-skill`은 명확한 에러로 거부**(에이전트에게
  스킬 저장을 시키라고 안내). 목록 조회/삭제(remove)는 지원.
- **tools** = `platform_toolsets.api_server` 리스트(우리가 렌더링하는 그 키)를 직접 편집.
  KNOWN_TOOLSETS = API_SERVER_TOOLSETS + 비기본(clarify/tts/computer_use/homeassistant/spotify/
  moa/video/x_search); 미지의 이름은 거부. toolset 결정은 프로세스 기동 시 → **재시작 필요**.
- **core** = config.yaml의 자유 스칼라 키(dotted 표기, 예: `agent.max_turns=100`; 값은
  yaml.safe_load로 파싱해 기록, 읽기는 safe_dump로 문자열화 → verify 일관). **보호 키**
  (`model*`, `approvals*`, `platform_toolsets`, `terminal.cwd`)는 caduceus 소유 — set 거부,
  core 뷰에서도 제외(model.api_key 시크릿 노출 방지). core 변경도 **재시작**.
- **CHANGE_KIND_STRATEGY 확정**: skills/soul → hot_reload(no-op; 다음 프롬프트 빌드에 반영),
  tools/core → restart_serve(stop→start→host_port 갱신→pooled transport 종료). 설계했던 seam이
  실측과 정확히 일치.
- **I/O**: `docker cp`(정지 컨테이너에도 동작; 읽기는 tar 스트림 파싱) read_file/write_file/
  list_dir + 스킬 삭제만 `docker exec rm -rf`(실행 중 필요). WriteConfig 시그니처를
  (rec, current, updated)로 확장(스킬 삭제 diff 계산). PyYAML 의존성 추가(자체 파서 금지).
